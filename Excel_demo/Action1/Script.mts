DataTable.AddSheet "sign_in_test_data_provided_by_srilaxmi"
 
DataTable.ImportSheet"C:\Users\GSRILAXM\Downloads\pravin_parameterization_example (4).xlsx","sign_in_test_me_app",3

number_of_records=DataTable.GetSheet("sign_in_test_data_provided_by_srilaxmi").GetRowCount
...
WpfWindow("devname:=Micro Focus MyFlight Sample Application").WpfEdit("devname:=agentName").Set username
WpfWindow("devname:=Micro Focus MyFlight Sample Application").WpfEdit("devname:=password").SetSecure password
'WpfWindow("devname:=HPE MyFlight Sample Application").WpfButton("name:=OK").Click
WpfWindow("devname:=Micro Focus MyFlight Sample Application").WpfButton("text:=Cancel").Click
wait(3)
